...
export default class TransactionScreen extends React.Component {
    ...
    searchTransactions = async(text)=>{
      ...
      if(search[0].toUpperCase() === "B"){
        ...
        const Squery = await db.collection("transaction").where("studentId","===",text).get()
        Squery.docs.map(doc=>{
          this.setState({
            allTransactions:[...this.state.allTransactions,doc.data()],
            lastTransactionVisible:doc
    ...
    fetchMoreTransaction = async()=>{
      ...
      if(enteredText[0] === "B"){
        const query = await db.collection("transaction").where("bookId","===",text).startAfter(this.state.lastTransactionVisible).limit(10).get()
        query.docs.map(doc=>{
          this.setState({
            allTransactions:[...this.state.allTransactions,doc.data()],
            lastTransactionVisible:doc
        ...
        const axe = await db.collection("transaction").where("studentId","===",text).startAfter(this.state.lastTransactionVisible).limit(10).get()
        axe.docs.map(doc=>{
          this.setState({
            allTransactions:[...this.state.allTransactions,doc.data()],
            lastTransactionVisible:doc
